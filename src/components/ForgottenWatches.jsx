...
export default function ForgottenWatches({ watches, history, onSelectWatch }) {
  ...
  return (
    <div className="mb-4 rounded-2xl border border-white/10 bg-neutral-900/40 p-4">
      <p className="text-sm font-semibold text-neutral-100">Esquecidos na caixa</p>
      <p className="mt-0.5 text-xs text-neutral-500">Relógios que estão pedindo pra sair um pouco.</p>
        ...
        {forgotten.map((w) => {
          ...
          return (
            <button
              ...
              className="flex w-32 shrink-0 flex-col items-start gap-2 rounded-xl border border-white/10 bg-black/20 p-3 text-left transition hover:border-amber-400/40"
            >
              <ColorSwatch hexes={w.hexes} />
              <p className="line-clamp-2 text-xs font-semibold text-neutral-100">{w.nome}</p>
