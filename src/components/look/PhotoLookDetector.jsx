  ...
  if (status === 'detected' && zones) {
    return (
      <div className="rounded-2xl border border-accent/35 bg-accent/5 p-4">
        <p className="text-sm font-semibold text-accent">Detectamos:</p>
        ...
        <div className="mt-3 flex gap-2">
          <button
            ...
            className="flex-1 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-bone transition hover:opacity-90"
          >
            Confirmar
          </button>
          <button
            onClick={onDone}
            className="flex-1 rounded-full border border-border px-4 py-2 text-sm font-medium text-text-muted transition hover:bg-surface-3"
          >
            Editar manualmente
          </button>
        </div>
    ...
    <div className="rounded-2xl border border-dashed border-accent/35 bg-accent/5 p-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <label htmlFor={inputId} className="cursor-pointer text-sm font-semibold text-accent">
